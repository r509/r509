...
describe R509::Csr do
    ...
    context "when passing a cert (single param) to create_with_cert" do
        ...
        it "san domains from the cert should be encoded in the request" do
            csr = R509::Csr.create_with_cert @cert_san
    ...
    context "when passing a list of domains to create_with_cert" do
        it "duplicates should be removed" do
            csr = R509::Csr.create_with_cert @cert, 2048, ['langui.sh','victoly.com','victoly.com','domain.local','victoly.com']
    ...
    context "when passing an array to create_with_subject" do
        it "generates a matching csr" do
            csr = R509::Csr.create_with_subject [['CN','langui.sh'],['ST','Illinois'],['L','Chicago'],['C','US'],['emailAddress','ca@langui.sh']]
        ...
        it "generates a matching csr with san domains" do
            csr = R509::Csr.create_with_subject [['CN','langui.sh'],['emailAddress','ca@langui.sh']],2048,['domain2.com','domain3.com']
        ...
        it "generates a matching csr when supplying raw oids" do
            csr = R509::Csr.create_with_subject [['2.5.4.3','common name'],['2.5.4.15','business category'],['2.5.4.7','locality'],['1.3.6.1.4.1.311.60.2.1.3','jurisdiction oid openssl typically does not know']]
    ...
    context "when setting alternate signature algorithms" do
        it "sets sha1 properly after setting to another hash" do
            csr = R509::Csr.create_with_subject [['CN','sha1-signature-alg.test']]
        ...
        it "sets sha256 properly" do
            csr = R509::Csr.create_with_subject [['CN','sha256-signature-alg.test']]
            csr.message_digest = 'sha256'
            csr.message_digest.should == 'sha256'
            csr.signature_algorithm.should == "sha256WithRSAEncryption"
        end
        it "sets sha512 properly" do
            csr = R509::Csr.create_with_subject [['CN','sha512-signature-alg.test']]
        ...
        it "sets md5 properly" do
            csr = R509::Csr.create_with_subject [['CN','md5-signature-alg.test']]
